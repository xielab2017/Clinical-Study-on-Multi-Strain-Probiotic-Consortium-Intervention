#Figure 5D,E Random forest model based on genus level
#Result Output result file name
#├── RFCV_result Summary of Microbial Species Level Random Forest Model Results
#│   ├── model Figure5D Random Forest Cross Validation Error Rate Curve
#│   ├── Imprortance Figure5E,S4D Importance assessment of random forest species under different random number seeds

library(EasyMicroPlot) 
df<-read.csv('level-6.csv',header = T)
mapping<-read.csv('IBSyazumapping.csv')
